#!/usr/bin/env bash
set -euo pipefail

install() {
  echo Hello World
  #!/bin/bash

  # Start
  echo "Starting"
  echo "Starting deployment operations..."

  # Load action configuration
  # Assuming the action configuration is stored in a YAML file
  action=$(cat /app/action.yaml)

  # Check if file exists
  filepath="/app/ubuntu.16.04-x64"
  if [ ! -d "$filepath/Microsoft.Fabric.Provisioning.Client" ]; then
      echo "File does not exist"
      exit 1
  fi

  echo "File exists"

  # Construct the command based on action type
  if [ "${action["Steps"][0]["Type"]}" == "Workspace" ] || [ -z "${action["Steps"][0]["Type"]}" ]; then
      # Construct payload for Workspace type
      payload='{"displayName":"'${action["Steps"][0]["DisplayName"]}'","description":"'${action["Steps"][0]["Description"]}'","capacityId":"'${action["Steps"][0]["CapacityId"]}'"}'
      cmd="$filepath/Microsoft.Fabric.Provisioning.Client create --token ${action["Steps"][0]["Token"]} --payload '$payload'"
  else
      # Construct payload for other types
      payload='{"displayName":"'${action["Steps"][0]["DisplayName"]}'","type":"'${action["Steps"][0]["Type"]}'","description":"'${action["Steps"][0]["Description"]}'","workspaceId":"'${action["Steps"][0]["WorkspaceId"]}'"}'
      cmd="$filepath/Microsoft.Fabric.Provisioning.Client createItem --token ${action["Steps"][0]["Token"]} --payload '$payload'"
  fi

  echo "Executing command: $cmd"
  output=$(eval "$cmd")
  if [ $? -ne 0 ]; then
      echo "Error executing command: $cmd"
      exit 1
  fi

  echo "$output"  # Output from the command

}

upgrade() {
  echo World 2.0
}

uninstall() {
  echo Goodbye World
}

view() {
  echo View 
}

# Call the requested function and pass the arguments as-is
"$@"
